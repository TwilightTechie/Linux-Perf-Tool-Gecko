...
class Thread:
	...
	def _intern_frame(self, frame_str: str) -> int:
		...
		category = KERNEL_CATEGORY_INDEX if frame_str.find('kallsyms') != -1 \
	...
	def _add_sample(self, comm: str, stack: List[str], time_ms: Milliseconds) -> None:
		...
		"""
		# threads may not set their names right after they are created.
...
def process_event(param_dict) -> GeckoProfile:
		global start_time
		...
		# parse the callchain of the current sample into a stack.
		stack = []
		if param_dict['callchain']:
			...
			if len(stack) != 0:
				# root come first and the leaf at the end.
				stack = stack[::-1]
...
def trace_end() -> Dict:
	...
	json.dump(gecko_profile_meta, sys.stdout, indent=2)
